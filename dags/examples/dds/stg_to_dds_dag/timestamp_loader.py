...
class TimestampsOriginRepository:
    ...
    def list_timestamps(self, timestamp_threshold: int, limit: int) -> List[TimestampObj]:
        with self._db.client().cursor(row_factory=class_row(TimestampObj)) as cur:
            cur.execute(
                """
                    SELECT id, object_value::JSON->>'date' AS ts, DATE_PART('year', (object_value::JSON->>'date')::TIMESTAMP)::int2 AS year, DATE_PART('month', (object_value::JSON->>'date')::TIMESTAMP)::int2 AS month,  DATE_PART('day', (object_value::JSON->>'date')::TIMESTAMP)::int2 AS day, TO_CHAR(TO_TIMESTAMP(object_value::JSON->>'date', 'YYYY-MM-DD"T"HH24:MI:SS"Z"'), 'YYYY-MM-DD') AS date, TO_CHAR(TO_TIMESTAMP(object_value::JSON->>'date', 'YYYY-MM-DD"T"HH24:MI:SS"Z"'), 'HH24:MI:SS') AS time
...
class TimestampLoader:
    WF_KEY = "example_timestamps_to_dds_workflow"
