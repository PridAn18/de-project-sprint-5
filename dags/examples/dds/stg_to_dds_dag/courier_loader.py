...
class CouriersOriginRepository:
    ...
    def list_couriers(self, courier_threshold: int, limit: int) -> List[CourierObj]:
        with self._db.client().cursor(row_factory=class_row(CourierObj)) as cur:
            cur.execute(
                """
                    select * from (select ROW_NUMBER() OVER (ORDER BY (SELECT 1))::int4 AS id, object_value::JSON->>'_id' AS courier_id, object_value::JSON->>'name' AS courier_name
                    FROM stg.apisystem_couriers) aa
...
class CourierLoader:
    WF_KEY = "example_couriers_to_dds_workflow"
