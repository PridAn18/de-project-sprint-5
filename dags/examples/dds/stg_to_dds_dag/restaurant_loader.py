...
class RestaurantsOriginRepository:
    ...
    def list_restaurants(self, restaurant_threshold: int, limit: int) -> List[RestaurantObj]:
        with self._db.client().cursor(row_factory=class_row(RestaurantObj)) as cur:
            cur.execute(
                """
                    SELECT id, object_value::JSON->>'_id' AS restaurant_id, object_value::JSON->>'name' AS restaurant_name, update_ts::text as active_from, '2099-12-31 00:00:00.000' as active_to
...
class RestaurantLoader:
    WF_KEY = "example_restaurants_to_dds_workflow"
